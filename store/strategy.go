...
type GridStrategyConfig struct {
	...
	// Volatility profile thresholds (adapt to different asset classes)
	// Regime classification: Bollinger Band width breakpoints (%)
	NarrowBBWidth   float64 `json:"narrow_bb_width,omitempty"`   // Upper limit for Narrow regime (crypto: 2.0, gold: 0.8)
	StandardBBWidth float64 `json:"standard_bb_width,omitempty"` // Upper limit for Standard regime (crypto: 3.0, gold: 1.5)
	WideBBWidth     float64 `json:"wide_bb_width,omitempty"`     // Upper limit for Wide regime (crypto: 4.0, gold: 2.5)
	// Regime classification: ATR% breakpoints
	NarrowATRPct   float64 `json:"narrow_atr_pct,omitempty"`   // Upper limit for Narrow regime (crypto: 1.0, gold: 0.3)
	StandardATRPct float64 `json:"standard_atr_pct,omitempty"` // Upper limit for Standard regime (crypto: 2.0, gold: 0.7)
	WideATRPct     float64 `json:"wide_atr_pct,omitempty"`     // Upper limit for Wide regime (crypto: 3.0, gold: 1.2)
	// AI prompt market regime thresholds
	RangingBBWidth  float64 `json:"ranging_bb_width,omitempty"`  // BB width indicating ranging market (crypto: 3.0, gold: 1.5)
	TrendingBBWidth float64 `json:"trending_bb_width,omitempty"` // BB width indicating trending market (crypto: 4.0, gold: 2.5)
	RangingEMADist  float64 `json:"ranging_ema_dist,omitempty"`  // EMA distance for ranging (crypto: 1.0, gold: 0.5)
	TrendingEMADist float64 `json:"trending_ema_dist,omitempty"` // EMA distance for trending (crypto: 2.0, gold: 1.0)
	// Grid breakout threshold
	BreakoutPausePct float64 `json:"breakout_pause_pct,omitempty"` // % beyond boundary to pause grid (crypto: 2.0, gold: 1.0)
}
