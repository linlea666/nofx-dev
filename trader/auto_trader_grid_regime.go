...
// resolveRegimePositionPct returns the position limit % for a regime level
// using defaults that work across asset classes (configurable via GridConfigModel DB fields).
func resolveRegimePositionPct(level market.RegimeLevel, _ *store.GridStrategyConfig) float64 {
	switch level {
...
// resolveRegimeLeverage returns the recommended leverage for a regime level
func resolveRegimeLeverage(level market.RegimeLevel, _ *store.GridStrategyConfig, currentLeverage int) int {
	switch level {
	case market.RegimeLevelNarrow:
		return min(currentLeverage, 2)
	case market.RegimeLevelStandard:
		return min(currentLeverage, 4)
	case market.RegimeLevelWide:
		return min(currentLeverage, 3)
	case market.RegimeLevelVolatile:
		return min(currentLeverage, 2)
	default:
		return min(currentLeverage, 2)
...
func (at *AutoTrader) GetGridRiskInfo() *GridRiskInfo {
	...
	// Calculate max position based on regime, using configurable thresholds
	regimeLevel := market.RegimeLevel(at.gridState.CurrentRegimeLevel)
	if regimeLevel == "" {
		regimeLevel = market.RegimeLevelStandard
	}

	maxPositionPct := resolveRegimePositionPct(regimeLevel, gridConfig)
	maxPosition := totalInvestment * maxPositionPct / 100 * float64(leverage)
	recommendedLeverage := resolveRegimeLeverage(regimeLevel, gridConfig, leverage)
