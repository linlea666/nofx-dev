...
func (at *AutoTrader) handleBreakout(breakoutType BreakoutType, breakoutPct float64) error {
	...
	pauseThreshold := 2.0
	if cfg := at.config.StrategyConfig.GridConfig; cfg != nil && cfg.BreakoutPausePct > 0 {
		pauseThreshold = cfg.BreakoutPausePct
	}

	if breakoutPct >= pauseThreshold {
		logger.Warnf("[Grid] Significant breakout (%.2f%% >= %.2f%%), pausing grid and canceling orders", breakoutPct, pauseThreshold)
	...
	minorThreshold := pauseThreshold / 2
	if breakoutPct >= minorThreshold {
